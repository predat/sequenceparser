...
typedef std::ptrdiff_t Time;
    ...
    #define SEQUENCEPARSER_ENUM_BITWISE_OPERATORS(ENUM_TYPE)                                                                                         \
                                                                                                                                                     \
        inline ENUM_TYPE operator~(const ENUM_TYPE& a) { return (ENUM_TYPE)(~int(a)); }                                                              \
                                                                                                                                                     \
        inline ENUM_TYPE operator&=(ENUM_TYPE& a, const ENUM_TYPE& b) { return a = (ENUM_TYPE)(int(b) & int(a)); }                                   \
                                                                                                                                                     \
        inline ENUM_TYPE operator&(const ENUM_TYPE& a, const ENUM_TYPE& b) { return (ENUM_TYPE)(int(b) & int(a)); }                                  \
                                                                                                                                                     \
        inline ENUM_TYPE operator|=(ENUM_TYPE& a, const ENUM_TYPE& b) { return a = (ENUM_TYPE)(int(b) | int(a)); }                                   \
                                                                                                                                                     \
        inline ENUM_TYPE operator|(const ENUM_TYPE& a, const ENUM_TYPE& b) { return (ENUM_TYPE)(int(b) | int(a)); }
    ...
    for (std::size_t i = 0; i < utf8.size();)
    {
        ...
        else if ((c & 0xE0) == 0xC0 && i + 1 < utf8.size())
        {
            ...
            if ((c2 & 0xC0) == 0x80)
            {
                ...
                latin1 += (codepoint <= 0xFF) ? static_cast<char>(codepoint) : '?';
                i += 2;
            }
            else
            {
